...
const SHARED_STYLES = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .nav{position:fixed;top:0;left:0;right:0;z-index:100;display:flex;align-items:center;justify-content:space-between;padding:16px 48px;background:rgba(10,10,10,0.92);backdrop-filter:blur(16px);box-shadow:0 1px 0 var(--border);animation:navDrop 0.6s ease 0.2s both;}  @keyframes navDrop{from{opacity:0;transform:translateY(-100%)}to{opacity:1;transform:translateY(0)}}
  ...
  .service-tag{display:inline-block;color:var(--orange);background:#fff;border:1.5px solid var(--orange);border-radius:4px;padding:4px 12px;margin-bottom:20px;}
  .hero-title{margin:0 0 20px;animation:fadeUp 0.7s ease 0.5s both;}
  .hero-desc{margin:0 0 36px;animation:fadeUp 0.7s ease 0.7s both;}
  ...
  .svc-link{padding:10px 20px;border-radius:8px;border:1px solid var(--border);color:var(--text-muted);text-decoration:none;transition:all 0.2s;}
...
export default async function ServicesElevatedPage() {
  ...
  const { data: carouselRows } = await supabase
    ...
    .eq("slot_key", "svc-elevated.carousel")
    .is("scope_id", null)
    ...
    <div style={{ background: "#0A0A0A", minHeight: "100vh" }}>
      ...
      <nav className="nav">
        ...
        <div className="nav-links">
          <a href="/clients" className="pg-btn">Clients</a><a href="/campaigns" className="pg-btn">Campaigns</a><a href="/about/team" className="pg-btn">About</a>
      ...
      <div className="hero-wrap">
        <div className="carousel-bg">
          {photos.map((photo, i) => (
            <div key={i} className={`carousel-slide${i === 0 ? " active" : ""}`}>
              <img className={i % 2 === 0 ? "ken-burns-a" : "ken-burns-b"} src={photo.url} alt="" style={{ objectPosition: photo.focal_point || '50% 20%', transform: photo.scale !== 1 ? `scale(${photo.scale})` : undefined }} />
            </div>
          ))}
        ...
        <div className="hero-glass-card">
          <div className="pg-eyebrow service-tag">Elevated NIL</div>
          <h1 className="pg-h1 hero-title">Premium.<br />Cinematic.<br />Unforgettable.</h1>
          <p className="pg-lead hero-desc">Elevated campaigns pair your brand with elite college athletes for high-production content that looks and feels like a national ad campaign.</p>
          <div className="hero-actions">
            <a href="/contact" className="pg-btn btn-solid">Start a Campaign</a>
            <a href="/campaigns" className="pg-btn btn-outline">See Examples</a>
          </div>
          <div className="hero-stats">
            <div><div className="pg-stat stat-num">50+<span></span></div><div className="pg-label stat-label">Headliner Athletes</div></div>
            <div><div className="pg-stat stat-num">4K<span></span></div><div className="pg-label stat-label">Production Standard</div></div>
            <div><div className="pg-stat stat-num">48<span>hr</span></div><div className="pg-label stat-label">Turnaround</div></div>
            <div><div className="pg-stat stat-num">100<span>%</span></div><div className="pg-label stat-label">Brand Approved</div></div>
          </div>
        </div>
      ...
      <section className="section" style={{ paddingTop: 0 }}>
        <div className="pg-eyebrow section-eyebrow">What&apos;s Included</div>
        <h2 className="pg-h2 section-title">The Elevated NIL Package</h2>
      ...
      <section className="cta">
        <h2 className="pg-h2 cta-title">Ready to Make<br />Something Great?</h2>
        <p className="pg-body cta-sub">Let&apos;s build a campaign that actually looks like your brand deserves.</p>
      ...
      <footer className="footer">
        <div className="footer-inner">
          <div className="footer-top">
            <div><p className="pg-body footer-brand-desc">The #1 NIL agency in the country. Connecting elite college athletes with the world&apos;s most ambitious brands.</p></div>
