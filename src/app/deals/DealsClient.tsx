...
type DealRow = Deal & {
  /** The deal's URL. NOT NULL in the database; the card links are built from it. */
  slug: string;
...
export default function DealsClient({
  ...
  /** brand_id -> resolved on_black logo url, resolved once on the server. */
  ...
  const focalMap = useMemo(() => Object.fromEntries(
    deals.filter(d => d.focal_point != null).map(d => [d.id, d.focal_point as string])), [deals]);
  const tabletFocalMap = useMemo(() => Object.fromEntries(
    deals.filter(d => d.focal_point_tablet != null).map(d => [d.id, d.focal_point_tablet as string])), [deals]);
  const mobileFocalMap = useMemo(() => Object.fromEntries(
    deals.filter(d => d.focal_point_mobile != null).map(d => [d.id, d.focal_point_mobile as string])), [deals]);

  /* ── Zoom maps (per device) ──────────────────────────────── */
  const zoomMap = useMemo(() => Object.fromEntries(
    deals.filter(d => d.zoom_desktop != null).map(d => [d.id, d.zoom_desktop as number])), [deals]);
  const tabletZoomMap = useMemo(() => Object.fromEntries(
    deals.filter(d => d.zoom_tablet != null).map(d => [d.id, d.zoom_tablet as number])), [deals]);
  const mobileZoomMap = useMemo(() => Object.fromEntries(
    deals.filter(d => d.zoom_mobile != null).map(d => [d.id, d.zoom_mobile as number])), [deals]);
  ...
  const resolvedLogos = useMemo(() => new Map(Object.entries(logoByBrand)), [logoByBrand]);

  // Data arrives as props from the server component. This page used to fetch
  // in the browser, which meant the deal ledger was invisible to crawlers and
  // the index could carry no metadata — see page.tsx.
  ...
  // 4:5, matching the grid cards. These were 248x380 (about 2:3) while the grid
  // was already aspect-[4/5], so the same photo was cropped two different ways
  // on one page — which is what produced the sliver-of-hoodie framing on the
  // taller carousel box.
  const carCardH = isMobile
    ? "calc(clamp(150px,42vw,200px) * 1.25)"
    : isTablet
    ? "calc(clamp(160px,28vw,220px) * 1.25)"
    : "310px";
          ...
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            ...
            <div style={{ overflow: "hidden" }}>
              <div style={{ display: "flex", gap: "clamp(12px,1.5vw,20px)", transition: "transform 0.5s ease", transform: `translateX(-${carIdx * (248 + 20) * 4}px)` }}>
                {featured.map(d => (
                  <Link key={d.id} href={`/deals/${d.slug}`} style={{ flex: `0 0 ${carCardW}`, width: carCardW, height: carCardH, borderRadius: "clamp(10px,1.3vw,16px)", overflow: "hidden", position: "relative", textDecoration: "none", color: "#fff", display: "block" }}>
      ...
      <div style={{ position: "relative", zIndex: 2, background: "#000", maxWidth: 1200, margin: "0 auto", padding: "clamp(32px,5vw,48px) clamp(20px,4vw,48px) clamp(48px,7vw,80px)" }}>
        {filtered.length === 0 ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: gridCols, gap: "clamp(12px,1.5vw,20px)" }}>
            {filtered.map(deal => (
              <Link key={deal.id} href={`/deals/${deal.slug}`} className="hover-lift" style={{ borderRadius: "clamp(10px,1.3vw,16px)", overflow: "hidden", border: "1px solid rgba(255,255,255,0.08)", background: "#111", textDecoration: "none", color: "#fff", display: "block" }}>
                {deal.image_url && (
