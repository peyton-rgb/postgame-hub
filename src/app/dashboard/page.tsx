...
const NAV_GROUPS: NavGroup[] = [
  {
    ...
    items: [
      ...
      {
        type: "link", key: "editing", label: "Editing Queue",
      ...
      {
        type: "link", key: "brand-approval", label: "Brand Approval",
        href: "/dashboard/brand-approval",
        icon: (
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  },
  {
    ...
    items: [
      {
        type: "link", key: "draft-graphics", label: "Draft Graphics",
        href: "/dashboard/graphic-creation/draft",
        icon: (
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  },
  {
    ...
    items: [
      {
        type: "link", key: "campaign-briefs", label: "Campaign Briefs",
        href: "/dashboard/campaign-briefs",
        icon: (
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", minHeight:400 }}>
      <div style={{ textAlign:"center", maxWidth:480 }}>
        ...
        <h2 style={{ fontSize:22, fontWeight:900, marginBottom:12, color:"var(--ink-1)" }}>{label}</h2>
        <p style={{ fontSize:15, color:"var(--ink-4)", lineHeight:1.6, marginBottom:28 }}>{desc}</p>
        <Link href={href} style={{ display:"inline-block", padding:"12px 28px", background:"var(--accent)", borderRadius:8, color:"var(--ink-1)", fontWeight:800, fontSize:13, textDecoration:"none", textTransform:"uppercase", letterSpacing:"0.07em" }}>
          Open {label} →
        </Link>
      </div>
    </div>
...
function DashboardContent() {
  ...
  return (
    <div className="min-h-screen flex">
      {/* ───── Sidebar ───── */}
      <aside
        className={`fixed top-0 left-0 h-full bg-ground border-r border-hairline flex flex-col z-50 transition-all duration-200 ${
          collapsed ? "w-16" : "w-60"
        }`}
      >
        ...
        <div className="flex items-center justify-between px-4 py-4 border-b border-hairline">
          ...
          <button
            ...
            className="text-ink-4 hover:text-ink-1 transition-colors p-1"
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-2">
          {NAV_GROUPS.map((group) => (
            <div key={group.label} className="mb-4">
              {!collapsed && (
                <div className="px-3 mb-1.5 text-[10px] font-bold uppercase tracking-widest text-ink-4">
                  {group.label}
                </div>
              )}
              {collapsed && <div className="border-t border-hairline mx-2 mb-2" />}
        ...
        <div className="border-t border-hairline p-2">
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-ink-4 hover:text-ink-1 hover:bg-surface-card transition-colors"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <main className={`flex-1 transition-all duration-200 ${collapsed ? "ml-16" : "ml-60"}`}>
        ...
        <div className="nav:border-b nav:border-hairline px-5 pt-5 pb-1 nav:px-8 nav:py-4 flex items-center justify-between">
...
export default function Dashboard() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-ink-4">Loading...</div>}>
      <DashboardContent />
    </Suspense>
  );
}
