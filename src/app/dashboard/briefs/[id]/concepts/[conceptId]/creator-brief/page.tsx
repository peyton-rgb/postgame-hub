...
import InspoPickerModal, { type SelectedInspoItem } from '@/components/InspoPickerModal';
...
export default function CreatorBriefEditorPage({
  ...
  // Inspo picker state for photos/videos sections
  const [inspoPickerSection, setInspoPickerSection] = useState<{
    index: number;
    type: string;
  } | null>(null);
  ...
  return (
    <div className="min-h-screen bg-[#07070a] text-white p-8">
          ...
          {creatorBrief.sections.map((section, index) => (
            <div key={`${section.type}-${index}`} className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              {section.type === 'shoot_logistics' ? (
                ...
                <>
                  {/* "Add from Library" button for photos and videos sections */}
                  {(section.type === 'photos' || section.type === 'videos') && (
                    <div className="mb-4">
                      <button
                        onClick={() => setInspoPickerSection({ index, type: section.type })}
                        className="px-4 py-2 bg-[#D73F09]/20 border border-[#D73F09]/30 hover:bg-[#D73F09]/30 text-[#D73F09] rounded-lg text-sm font-medium transition-colors"
                      >
                        + Add from Inspo Library
                      </button>
                    </div>
                  )}
      ...
      {/* Inspo Picker Modal for photos/videos sections */}
      <InspoPickerModal
        isOpen={!!inspoPickerSection}
        onClose={() => setInspoPickerSection(null)}
        filterContentType={
          inspoPickerSection?.type === 'photos' ? 'photography' : undefined
        }
        onSelect={(items: SelectedInspoItem[]) => {
          if (!inspoPickerSection || !creatorBrief) return;
          const sectionIndex = inspoPickerSection.index;
          const section = creatorBrief.sections[sectionIndex];
          const content = section.content as Record<string, unknown>;

          if (inspoPickerSection.type === 'photos') {
            // Add to images array
            const existing = (content.images as Array<{ url: string; caption: string }>) || [];
            const newImages = items.map((item) => ({
              url: item.file_url || item.thumbnail_url || '',
              caption: item.visual_description || '',
            }));
            updateSection(sectionIndex, {
              ...section,
              content: { ...content, images: [...existing, ...newImages] },
            });
          } else if (inspoPickerSection.type === 'videos') {
            // Add to videos array
            const existing = (content.videos as Array<{ url: string; caption: string }>) || [];
            ...
            updateSection(sectionIndex, {
              ...section,
              content: { ...content, videos: [...existing, ...newVideos] },
            });
          }
          setInspoPickerSection(null);
        }}
      />
    </div>
  );
