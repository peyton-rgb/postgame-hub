// ============================================================
// Brief Intake Form — /dashboard/briefs/new
...
// Campaign type options with human-readable labels
...
export default function NewBriefPage() {
  ...
  // Brief content as plain text (simplified — replace with Tiptap later)
  const [briefText, setBriefText] = useState('');

  // Upload brief state (AI parsing)
  const [briefInputMode, setBriefInputMode] = useState<'manual' | 'upload'>('manual');
  ...
  // Load brands for the autocomplete
  useEffect(() => {
  ...
  // Handle brief document upload → AI parsing
  async function handleBriefUpload(file: File) {
    ...
    try {
      ...
      setParsedBrief(fields);

      // Pre-populate the form with parsed fields
      if (fields.name) updateField('name', fields.name);
      if (fields.campaign_type) updateField('campaign_type', fields.campaign_type);
      if (fields.raw_summary) setBriefText(fields.raw_summary);
      if (fields.mandatories) setMandatoriesText(fields.mandatories.join(', '));
      if (fields.restrictions) setRestrictionsText(fields.restrictions.join(', '));
      if (fields.deliverables) {
        // Append deliverables to brief text
        setBriefText((prev) =>
          prev + (prev ? '\n\n' : '') + 'Deliverables: ' + fields.deliverables.join(', ')
        );
      }
      if (fields.deadlines) {
        const deadlines = fields.deadlines as Record<string, string>;
        if (deadlines.shoot_date || deadlines.start_date) {
          updateField('start_date', deadlines.shoot_date || deadlines.start_date || '');
        }
        if (deadlines.launch || deadlines.launch_date) {
          updateField('target_launch_date', deadlines.launch || deadlines.launch_date || '');
        }
      }
      if (fields.athlete_notes) {
        setTargeting((prev) => ({ ...prev, notes: fields.athlete_notes }));
      }
    } catch (err) {
  ...
  async function publishBrief() {
    ...
    try {
      ...
      // Check side effects for warnings
      ...
      if (warnings.length > 0) {
        // Still published, but with warnings
          ...
          {currentStep === 3 && (
            <div className="space-y-6">
              ...
              <div className="flex gap-3 mb-2">
                <button
                  onClick={() => setBriefInputMode('manual')}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                    briefInputMode === 'manual'
                      ? 'bg-[#D73F09] text-white'
                ...
                <button
                  onClick={() => setBriefInputMode('upload')}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              ...
              {briefInputMode === 'upload' && (
                <div className="space-y-4">
                  <p className="text-gray-400 text-sm">
                    Drop a brand brief PDF or DOCX and the AI will extract campaign details, mandatories, restrictions, and deadlines automatically. You can review and edit everything before saving.
                  </p>
                  ...
                  >
                    ...
                    {uploadingBrief ? (
                      <div>
                        <div className="w-10 h-10 border-4 border-[#D73F09]/30 border-t-[#D73F09] rounded-full animate-spin mx-auto mb-3" />
                        <p className="text-gray-300 font-medium">Parsing brief with AI...</p>
                        <p className="text-gray-500 text-sm mt-1">This usually takes 10-30 seconds</p>
                      </div>
                    ) : (
                      <div>
                        <p className="text-gray-300 font-medium">
                          Drop a brief document here, or click to browse
                        </p>
                  ...
                  {parsedBrief && (
                    <div className="bg-green-900/20 border border-green-800/50 rounded-xl p-4">
                      ...
                      {parsedBrief.raw_summary && (
                        <p className="text-gray-400 text-sm italic">&quot;{parsedBrief.raw_summary as string}&quot;</p>
                      )}
                    </div>
                  )}
                </div>
              ...
              {/* Manual fields (always shown so they can be edited after upload too) */}
              <div>
                <label className="block text-sm text-gray-400 mb-2">
                  {briefInputMode === 'upload' && parsedBrief
                    ? 'Brief Content (pre-filled from upload — edit as needed)'
