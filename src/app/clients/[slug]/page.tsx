...
export default function BrandGalleryPage() {
  ...
  // We check BOTH the "campaigns" table AND the "campaign_recaps" table,
  // because Drive-imported campaigns only exist in campaign_recaps.
  useEffect(() => {
    if (!brand) {
      setLoading(false);
      return;
    ...
    async function fetchCampaigns() {
      ...
      // ── Source 1: campaigns table (legacy uploads) ──
      let legacyCampaigns: Campaign[] = [];
      ...
      if (campaignData && campaignData.length > 0) {
        ...
        if (dbBrand) {
          ...
          if (brandCampaigns) {
            legacyCampaigns = brandCampaigns;
          }
        }
      }
      ...
      // ── Source 2: campaign_recaps table (Drive imports / recap editor) ──
      // Find the brand_id in the brands table first
      const { data: recapBrand } = await supabase
        .from('brands')
        .select('id')
        .ilike('slug', slug)
        .single();

      let recapBrandId = recapBrand?.id;

      // Also try matching by client_name if slug didn't work
      if (!recapBrandId) {
        const { data: brandByName } = await supabase
          ...
          .single();
        recapBrandId = brandByName?.id;
      }

      if (recapBrandId) {
        const { data: recaps } = await supabase
          .from('campaign_recaps')
          .select('id, name, slug, description, hero_image_url, client_name, brand_id, created_at')
          .eq('brand_id', recapBrandId)
          .order('created_at', { ascending: false });

        if (recaps) {
          for (const r of recaps) {
            // Skip if we already have this campaign from the campaigns table
            if (seenSlugs.has(r.slug)) continue;
            seenSlugs.add(r.slug);

            // If no hero_image_url, try to grab the first image from media table
            let heroUrl = r.hero_image_url;
            if (!heroUrl) {
              const { data: firstMedia } = await supabase
                ...
                .limit(1)
                .single();
              heroUrl = firstMedia?.file_url || null;
            }
      ...
      setCampaigns(merged);
      await fetchVideosForCampaigns(legacyCampaigns, supabase);
      setLoading(false);
    }

    // For legacy campaigns, check if there are .mp4 video files in storage
