...
export default function PressContent({ articles }: { articles: PressArticle[] }) {
  ...
  const nonFeaturedCount = filtered.filter(a => !a.featured).length;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f4f0", fontFamily: "Arial,Helvetica,sans-serif" }}>
      ...
      {/* ── Filter Chips ───────────────────────── */}
      <div style={{ background: "#fff", borderBottom: "1px solid #ececec", padding: "14px 18px" }}>
            ...
            <a href={featuredArticle.external_url || "#"} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", color: "inherit", display: "block", background: "#fff", borderRadius: 14, border: "1px solid #e8e8e8", overflow: "hidden" }}>
              ...
              <div style={{ padding: 18 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  {featuredArticle.featured && <span style={{ fontSize: 9, fontWeight: 800, textTransform: "uppercase", background: "#D73F09", color: "#fff", padding: "3px 8px", borderRadius: 4 }}>Featured</span>}
                  {featuredArticle.publication && <span style={{ fontSize: 10, color: "#aaa", fontWeight: 600 }}>{featuredArticle.publication}</span>}
                </div>
                <h2 style={{ fontSize: isDesktop ? 22 : 17, fontWeight: 900, color: "#111", lineHeight: 1.25, margin: "0 0 6px" }}>{featuredArticle.title}</h2>
                <div style={{ fontSize: 11, color: "#999", marginBottom: 8 }}>{formatDate(featuredArticle.published_date, featuredArticle.created_at)}</div>
        ...
        {latest.length > 0 && (
          <>
            <div style={{ padding: "20px 18px 10px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <span style={{ fontSize: 10, color: "#aaa" }}>{nonFeaturedCount} articles</span>
            </div>
            <div style={{ padding: "0 18px", display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
              {latest.map(a => (
                <a key={a.id} href={a.external_url || "#"} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", color: "inherit", display: "flex", gap: 12, padding: 12, alignItems: "flex-start", background: "#fff", borderRadius: 12, border: "1px solid #e8e8e8" }}>
                  {a.image_url ? (
                    <div style={{ width: 72, aspectRatio: "4/5", flexShrink: 0, borderRadius: 8, overflow: "hidden" }}>
                      <img src={a.image_url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                    </div>
                  ) : (
                    <div style={{ width: 72, aspectRatio: "4/5", borderRadius: 8, background: "#f0f0f0", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 900, color: "#ccc" }}>{a.title.slice(0, 2).toUpperCase()}</div>
                  )}
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: "#aaa", fontWeight: 600 }}>
                      {a.publication && <>{a.publication} · </>}{formatDate(a.published_date, a.created_at)}
                    </div>
                  </div>
                </a>
              ))}
              ...
              {grid.map(a => (
                <a key={a.id} href={a.external_url || "#"} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", color: "inherit", background: "#fff", borderRadius: 12, border: "1px solid #e8e8e8", overflow: "hidden", display: "block" }}>
                  {a.image_url ? (
                    ...
                    <div style={{ width: "100%", aspectRatio: "4/5", background: "#f0f0f0", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 900, color: "#ddd" }}>{a.title.slice(0, 2).toUpperCase()}</div>
                  )}
                  <div style={{ padding: 10 }}>
                    {a.category && <div style={{ fontSize: 8, fontWeight: 800, textTransform: "uppercase", color: "#D73F09", marginBottom: 3 }}>{a.category}</div>}
                    <div style={{ fontSize: 9, color: "#aaa", marginBottom: 3 }}>{formatDate(a.published_date, a.created_at)}</div>
