...
export async function GET() {
  ...
  if (campaignIds.length) {
    const { data: recaps } = await svc
      ...
      // Logo AND name come from the linked brand, not campaign_recaps: the
      // logo column there is set on 2 of 611 campaigns, and client_name is free
      // text, so the left rail would group "McDonalds" apart from "McDonald's".
      // Nested through the campaigns_brand_id_fkey FK, so no 2nd query.
      .select(
    ...
    return {
      ...
      campaign: recap
        ? {
            id: recap.id,
            name: recap.name,
            // Fall back to the free-text client_name only when no brand is
            // linked, so a campaign never renders brandless.
            brandId: recap.brand?.id ?? null,
            brandName: recap.brand?.name ?? recap.client_name,
