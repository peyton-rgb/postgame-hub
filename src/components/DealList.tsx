        ...
        viewMode === "card" ? (
          ...
          <div className="flex flex-col gap-2">
            {deals.map((deal) => (
              ...
              >
                <Link href={`/dashboard/deals/${deal.id}`} className="absolute inset-0 z-0" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3">
                    <h3 className="text-sm font-bold truncate">{deal.brand_name}</h3>
                    {deal.athlete_name && (
                      <span className="text-xs text-gray-400 truncate">{deal.athlete_name}</span>
                    )}
                    <span
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-xs text-gray-500 uppercase">{deal.tier.replace("_", " ")}</span>
                ...
                <button
                  ...
                  className="relative z-10 w-7 h-7 rounded-lg flex items-center justify-center text-gray-600 hover:text-status-bad-ink hover:bg-status-bad/10 opacity-0 group-hover:opacity-100 transition-all shrink-0"
                  title="Delete deal"
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <polyline points="3 6 5 6 21 6" />
