...
import StageMatrix, { type MatrixCampaign } from "@/components/portal/StageMatrix";
import RosterGrid from "@/components/portal/RosterGrid";
import Link from "next/link";

// Brand portal DASHBOARD BODY — hero, stat bar, in-flight matrix, campaign
// card strip, notable roster, assets two-up.
//
// Extracted from /portal/[token]/page.tsx unchanged in substance. It now
// takes an already-resolved brand and a basePath instead of a token, so
// the identical dashboard renders for a token visitor and for a
// signed-in brand user. The design is not forked.
//
// Service-role rationale (drafts must be visible): see src/lib/portal-data.ts.
// Whoever resolves the brand owns the gate — this body assumes the caller
// has already decided the viewer is entitled to see it.
...
export default async function PortalDashboardBody({
  brand,
  basePath,
}: {
  brand: PortalBrand;
