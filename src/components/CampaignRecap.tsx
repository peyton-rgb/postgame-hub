              ...
              {visibleRosterAthletes.map((a, i) => {
                ...
                <div key={a.id} className="flex items-center gap-3 py-3 px-3 rounded-lg bg-white/[0.05] border border-white/[0.10]">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Only recaps that actually carry month values get the flex meta row.
                        Everywhere else this stays the original plain <div>, so the other 81
                        recaps render byte-identical markup. The 50% floor is what drops the
                        pill onto its own line on narrow phones instead of crushing the school
                        text to ~36px; as a percentage it can never overflow the meta line the
                        way a fixed rem floor does at 320px. */}
                    {showCol("month") && hasAnyMonth ? (
