      ...
      <div style={{ marginTop: 4 }}>
        {!url ? (
          <button onClick={generate} disabled={loading}
            style={{ border: "1px solid var(--hairline)", background: "transparent", borderRadius: 8, padding: "7px 12px", fontSize: 12, color: "var(--ink-2)", cursor: "pointer" }}>
            {loading ? "Generating…" : "Generate videographer link"}
          </button>
        ) : (
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            <input readOnly value={url} onFocus={(e) => e.currentTarget.select()}
              style={{ flex: 1, minWidth: 220, background: "var(--surface-card)", border: "1px solid var(--hairline)", borderRadius: 8, padding: "7px 10px", color: "var(--ink-1)", fontSize: 12 }} />
            <button onClick={copy} style={{ background: "var(--accent)", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 12, color: "var(--ink-1)", fontWeight: 700, cursor: "pointer" }}>
              {copied ? "Copied!" : "Copy"}
            </button>
          </div>
        )}
    ...
    <div>
      {!url ? (
        ...
        <div className="a-card" style={{ textAlign: "left" }}>
          <div style={{ fontSize: 11, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--ink-4)", marginBottom: 6 }}>Videographer link</div>
